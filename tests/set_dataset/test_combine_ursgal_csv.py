...
def test_combine_ursgal_simple():
    df = peptideForest.setup_dataset.combine_ursgal_csv_files(path_dict_small)
    assert df['engine'].unique() == 'mascot'
    assert df['Mascot:Score'].equals(df['Score'])

def test_combine_ursgal_drops_proper_columns():
    df = peptideForest.setup_dataset.combine_ursgal_csv_files(path_dict_small)
    for col in peptideForest.knowledge_base.parameteres['columns_to_be_removed_from_input_csvs']:
