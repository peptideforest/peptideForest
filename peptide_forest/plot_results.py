...
from .analyse_results import get_protein_maxarea_dataset, get_num_psms_by_method,\
  get_num_psms_against_q_cut, get_top_targets, get_ranks
from .quant_data_verification import get_unique_sequence_counts_all,\
  calc_all_ratios, get_ratios_method, get_logn_mean_error, atan_func, fit_function,\
...
def plot_psm_numbers(psms: Dict, psms_avg: Dict, psms_engine: Dict, psms_other: Dict = None,
                     plot_train: bool = True, plot_val: bool = False,
                     perc_file: str = None, perc_val_file: str = None,
                     align_horizontal: bool = False, output_file: str = None,
                     dpi: int = 300, show_plot: bool =True) -> None:
  '''
  ...
  if plot_val:
    if align_horizontal:
      ...
      plt.figure(num=None, figsize=(15, 15), dpi=dpi, facecolor='w', edgecolor='k')
      plt.subplot(2, 1, 1)
  else:
  ...
  for model in models:
    ...
    if plot_train:
      ...
      plt.plot(x, psms[model]['train'],
               linewidth=5,
               linestyle='--',
               color=colors[model],
               label=f'{model_sub} - train')
      plt.plot(x_avg, psms_avg[model]['train'],
               linewidth=5,
               color=colors[model])
      test_label += ' - test'
    ...
    plt.plot(x, psms[model]['test'],
             linestyle=':',
             linewidth=5,
             color=colors[model],
             label=test_label)
    plt.plot(x_avg, psms_avg[model]['test'],
             linewidth=5,
             color=colors[model])
  ...
  if psms_other:
    for k, v in psms_other.items():
      plt.plot(x_other, [v, v], linestyle='-.',
               linewidth=5, label=k)
  ...
  for engine, data in psms_engine.items():
    ...
    for k, v in data.items():
      if plot_train:
        label = f'{label} - {k}'
      if k != 'train':
        plt.plot(x_other, [v for i in x_other],
                 linewidth=5,
                 linestyle='-.',
                 color=colors[engine],
                 label=label)
        ...
        if plot_train:
          plt.plot(x_other, [psms_engine['train'] for i in x_other],
                   linewidth=5,
                   linestyle='--',
                   color=colors[engine],
                   label=label)
  ...
  if perc_file:
    ...
    if plot_train:
      y = [p for p in perc_train] + [perc_train[-1] for i in
                                     np.arange(max_len_x - len(perc_train))]
    ...
    y = [p for p in perc_test] + [perc_test[-1] for i in np.arange(max_len_x - len(perc_test))]
    plt.plot(np.arange(1, len(y) + 1),
             y,
             linewidth=5,
             color='black',
    ...
    y = [p for p in perc_test_non_redundent] + [perc_test_non_redundent[-1]
  ...
  plt.legend(bbox_to_anchor=(0.0, -0.15), loc=2, borderaxespad=0., fontsize=16, ncol=3)
  ...
  if plot_val:
    if align_horizontal:
      plt.subplot(1, 2, 2)
      plt.title('Number of Target PSMs identified at each iteration: unseen data', fontsize=18)
    else:
      plt.subplot(2, 1, 2)
    plt.plot(x, [psms_engine['val'] for i in x],
             linewidth=5,
             color='orange',
             linestyle='--',
             label='engine - val')
    if perc_val_file:
      ...
      plt.plot(x, [p for p in perc_val_test] + [perc_val_test[-1] for i in
                                                np.arange(len(x) - len(perc_val_test))],
               linewidth=5,
               color='black',
               linestyle='--',
    ...
    for model in models:
      model_sub = model.split(' -')[0]
      plt.plot(x, psms[model]['val'],
               linestyle='--',
               linewidth=5,
               color=colors[model],
               label=f'{model_sub} - val')
      plt.plot(x_avg, psms_avg[model]['val'],
               linewidth=5,
               color=colors[model])

    plt.legend(bbox_to_anchor=(1.005, 0.95), loc=2, borderaxespad=0., fontsize=18)
    plt.xlabel('iterations', fontsize=18)
    plt.ylabel('PSMs below q=1%', fontsize=18)
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
...
def plot_accuracies(scores: Dict[str, Dict[str, Dict]],
                    scores_1pc: Dict[str, Dict[str, Dict]] = None) -> None:
  """
  ...
  if scores_1pc:
    for split in splits:
      ...
      plt.xticks(fontsize=16)
      plt.yticks(fontsize=16)
      plt.title(f'Accuracy of model for each iteration, q < 1%: {split} split', fontsize=18)
    plt.show()


def plot_ranks(df: pd.DataFrame, x_name: str, y_name: str, use_top_psm: bool = True,
               n_psms: int = 6, output_file: str = None, dpi: int = 300, show_plot: bool = True) -> None:
  """
  ...
  if use_top_psm:
    df_plot = df.sort_values(f'Score_processed_{y_name}',
  ...
  plt.plot(x, y, linestyle='None', marker='.', ms=4, color='orange', alpha=0.75, zorder=1,
           label='Top Targets')

  # plot other targets
  x = df_plot.loc[~df_plot[tt_y] & ~df_plot['Is decoy'], col_x]
  y = df_plot.loc[~df_plot[tt_y] & ~df_plot['Is decoy'], col_y]
  plt.plot(x, y, linestyle='None', marker='.', ms=4, color='red', alpha=0.75, zorder=2,
           label='Targets')
  ...
  plt.plot(x, y, linestyle='None', marker='.', ms=4, color='cyan', alpha=0.75, zorder=3,
           label='Former Top Targets')

  # plot decoys
  x = df_plot.loc[~df_plot[tt_y] & df_plot['Is decoy'], col_x]
  y = df_plot.loc[~df_plot[tt_y] & df_plot['Is decoy'], col_y]
  plt.plot(x, y, linestyle='None', marker='.', ms=4, color='blue', alpha=0.75, zorder=3,
           label='Decoys')

  # make a legend
  lgnd = plt.legend(bbox_to_anchor=(1.005, 0.95), loc=2, borderaxespad=0., fontsize=14)
...
def plot_maxarea(df: pd.DataFrame, prot: str) -> None:
  ...
  g = sns.catplot(x="ISOTOPELABEL_ID",
                  y="MAXAREA",
                  hue="Sequence",
                  col="method",
                  data=df_maxarea,
                  kind="bar",
                  height=8, aspect=1.1, col_wrap=2)
  plt.subplots_adjust(top=0.9)
  g.fig.suptitle(f'Protein: {prot[0:50]}', fontsize=18)
  plt.show()


def plot_channel_sum_against_ratio(df: pd.DataFrame,
                                   i: int, j: int,
                                   prots: List = None, output_file: str = None,
  ...
  x_min = min(data_plot['Human']['all_data'][cols].min().min(),
              data_plot['E-Coli']['all_data'][cols].min().min())
  x_max = max(data_plot['Human']['all_data'][cols].max().max(),
              data_plot['E-Coli']['all_data'][cols].max().max())
  y_min = min(data_plot['Human']['all_data'][cols_sum].min().min(),
              data_plot['E-Coli']['all_data'][cols_sum].min().min())
  y_max = max(data_plot['Human']['all_data'][cols_sum].max().max(),
              data_plot['E-Coli']['all_data'][cols_sum].max().max())

  for k, v in data_plot.items():

    with PdfPages(f'{output_file}_{k}.pdf') if output_file else dummy_context_mgr() as pp:
      for k1, v1 in v.items():
        plt_tmp = plt.figure(num=None, figsize=(10, 7), dpi=300, facecolor='w', edgecolor='k')
        plt.title("\n".join(wrap(k1, 80)).replace('<|>', '\n\n') + "\n", fontsize=18)
        colors = {'mascot': 'purple',
                  'msgfplus': 'orange',
                  'SVM': 'green',
                  'RF': 'black'}
        markers = {'mascot': 'v',
        ...
        for m in ['mascot', 'msgfplus', 'SVM', 'RF']:
          plt.plot(v1[f'ratio_{i}_{j}_{m}'].values,
                   v1[f'sum_{i}_{j}_{m}'].values,
                   color=colors[m], ms=mss[m], mew=mews[m],
                   marker=markers[m], linestyle='None', label=f'{m}')
        plt.legend(bbox_to_anchor=(1.005, 0.95), loc=2, borderaxespad=0., fontsize=14)
...
def plot_num_psms_by_method(df: pd.DataFrame, q_val_cut: float = 0.01,
                            methods: List = None, output_file: str = None,
                            print_values: bool = False, dpi: int = 300,
...
def plot_num_psms_against_q(df: pd.DataFrame, q_val_cuts: List = None,
                            methods: List = None, from_method: str = None,
                            output_file: str = None, dpi: int = 300, show_plot: bool = False) -> None:
  """
  Plot the number of PSMs for each method that we have results for
  Arguments:
    - df: dataframe containing results from search engines and ML training
  ...
  if q_val_cuts is None:
    q_val_cuts = sorted([float(f'{i}e-{j}') for i in np.arange(1, 10)
                         for j in np.arange(4, 1, -1)]) + [1e-1]

  # get dataframe for number of PSMs as a function of q-value
  df_num_psms_q = get_num_psms_against_q_cut(df, methods=methods, from_method=from_method,
                                             q_val_cuts=q_val_cuts)

  # markers to be used for plotting
  markers_base = ['s', 'o', 'v', 'x', '^', 'P', 'd', "h", "*",
                  "1", "2", "3", "4", "<", ">", "H", "p", "P"]
  ...
  plt.legend(bbox_to_anchor=(0.0, -0.15), loc=2, borderaxespad=0., fontsize=18, ncol=5)
...
def plot_ratio_against_sum(df: pd.DataFrame, sum_cut: float = 0.0,
                           n_slices: int = 15, quant_cut: bool = True,
  ...
  for q_val_cut in q_val_cuts:
    ...
    for icol, jcol in ratios:
      ...
      # initiate the plot
      plt.figure(num=None, figsize=(13, 8), dpi=dpi, facecolor='w', edgecolor='k')
      plt.title(f'Ratio of channels {icol} and {jcol} against sum of channels: '
                f'E-Coli, q_val_cut={q_val_cut}', fontsize=18)

      # get data for method_0, use bins to bin the other methods
      # get the ratio between channels icol and jcol
      if q_val_cut_fixed:
        df_ratio_0 = get_ratios_method(df_fixed,
                                       method=None,
                                       i=icol, j=jcol,
                                       drop_no_data=True,
                                       log2=True)
      ...
      df_means, bins = get_logn_mean_error(df_ratio,
                                           base=base,
                                           quant_cut=quant_cut,
                                           n_slices=n_slices,
      ...
      if q_val_cut_fixed:
        label = f'{method_0}: all PSMs at {q_val_cut_fixed}: {len(df_ratio)} PSMs'
      else:
        label = f'{method_0}: all PSMs at {q_val_cut}: {len(df_ratio)} PSMs'
      plt.errorbar((1e7 / df_means[x_col]), df_means[y_col], df_means[yerr_col],
                   ms=14, marker=markers_base[0], label=label, color='black',
                   linestyle='None')

      if plot_fits:
        mean_ratio_0, error, popt = fit_function(df_means)
        plt.plot((1e7 / df_means[x_col]),
                 atan_func(df_means[x_col],
                           popt[0],
                           popt[1],
                           popt[2]),
                 linewidth=4, color='black', label='_nolegend_')
        print(f'{method_0}: asymptote = {round(mean_ratio_0, 3)} +/ {round(error, 3)}')
      ...
      for i, method in enumerate(methods):
        ...
        sum_col = df_ratio.filter(like='sum').columns[0]
        df_ratio = df_ratio[df_ratio[sum_col] >= sum_cut]
        df_means = get_logn_mean_error(df_ratio,
                                       base=base,
                                       quant_cut=quant_cut,
                                       n_slices=n_slices,
                                       bins_in=bins,
                                       bin_log_sum=bin_log_sum,
                                       mean_of_log=mean_of_log)[0]
        y_col = df_means.filter(like='ratio').columns[0]
        yerr_col = df_means.filter(like='error').columns[0]
        x_col = df_means.filter(like='sum').columns[0]

        plt.errorbar((1e7 / df_means[x_col]), df_means[y_col], df_means[yerr_col],
                     ms=14, marker=markers_base[i + 1],
        ...
        if plot_fits:
          ...
          plt.plot((1e7 / df_means[x_col]),
                   atan_func(df_means[x_col],
                             ...
                             popt[1],
                             popt[2]),
                   linewidth=4, color=colors[i + 1], label='_nolegend_')
          print(f'{method}: asymptote = {round(mean_ratio_0, 3)} +/ {round(error, 3)}')
        ...
        if q_val_cut_fixed:
          ...
          #print(len(df_ratio))
          df_ratio = df_ratio.merge(df_ratio_0[[f'top_target_{method_0}']],
                                    left_index=True,
                                    right_index=True,
                                    how='inner')
        ...
        plt.errorbar((1e7 / df_means[x_col]), df_means[y_col], df_means[yerr_col],
                     ms=14, marker=markers_base[i2 + 1],
        ...
        if plot_fits:
          ...
          plt.plot((1e7 / df_means[x_col]),
                   atan_func(df_means[x_col],
                             ...
                             popt[1],
                             popt[2]),
                   linewidth=4, color=colors[i2 + 1], label='_nolegend_')
          print(f'{method}: only additional PSMs: asymptote = {round(mean_ratio_0, 3)}'
                f' +/ {round(error, 3)}')

      handles, labels = plt.gca().get_legend_handles_labels()
      nin = len(methods)
      order = np.arange(1, 2 * nin, 2).tolist() + [0] + np.arange(2, 2 * nin + 1, 2).tolist()
      plt.legend([handles[idx] for idx in order], [labels[idx] for idx in order],
                 bbox_to_anchor=(0.0, -0.15), loc=2, borderaxespad=0., fontsize=18, ncol=2)
      plt.xscale('log')
      ...
      if output_file:
        ...
        output_file_out = f'{file_base}-{icol}_{jcol}-{q_val_cut_print}.{file_format}'
        plt.savefig(output_file_out, format=file_format, dpi=dpi)
      plt.show()


# pylint: disable=too-many-arguments
def plot_plateau_against_q(df: pd.DataFrame, sum_cut: float = 0.0,
                           n_slices: int = 15, quant_cut: bool = True,
                           bin_log_sum: bool = False,
                           mean_of_log: bool = False,
                           methods: List = None,
  ...
  df_mean_plateau = get_ratio_against_sum(df, sum_cut=sum_cut,
                                          n_slices=n_slices, quant_cut=quant_cut,
                                          bin_log_sum=bin_log_sum,
                                          mean_of_log=mean_of_log,
                                          methods=methods,
                                          method_0=method_0,
  ...
  for i, method in enumerate(methods):
    ...
    plt.errorbar(x, y, yerr,
                 ms=14, marker=markers_base[i], label=f'{method}')
  plt.legend(bbox_to_anchor=(0.0, -0.15), loc=2, borderaxespad=0., fontsize=18, ncol=3)
  plt.xscale('log')
  plt.xlabel('q-value cut-off', fontsize=16)
  plt.ylabel(plot_col, fontsize=16)
  plt.xticks(fontsize=16)
  plt.yticks(fontsize=16)
  plt.title(f'Extrapolated ratio for channels {icol} and {jcol}: '
            f'E-Coli', fontsize=18)
  plt.tight_layout()
  if output_file:
    file_format = output_file.split('.')[-1]
    plt.savefig(output_file, format=file_format, dpi=dpi)
