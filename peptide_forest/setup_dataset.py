...
def make_dataset(
  ...
  """
    ...
    # get the features
    df, old_cols, feature_cols = get_features(
        df,
        only_top=only_top,
...
def get_features(
  ...
  """
    ...
    df = preprocess_features.calc_features(
        df,
        only_top=only_top,
        balance_dataset=balance_dataset,
        combine_engines=combine_engines,
...
def replace_missing_data_cv(
    train_j: pd.DataFrame, train_k: pd.DataFrame, test: pd.DataFrame
) -> pd.DataFrame:
    """
...
def get_top_target_decoy(
    df: pd.DataFrame, score_col: str, balanced: bool = False
) -> pd.DataFrame:
    """
...
def write_percolator_input_files(
    df: pd.DataFrame, feature_cols: List, file_name: str
) -> None:
    """
  ...
  """
    df_out = df.loc[
        :, ["Spectrum ID", "Is decoy"] + feature_cols + ["Sequence", "Protein ID"]
    ]
    ...
    bool_cols = bool_cols + [
        ...
        if "True" in df_out[c].astype(str).values
    ...
    df_top_line = pd.DataFrame(np.zeros([1, df_out.shape[1]]), columns=df_out.columns)
