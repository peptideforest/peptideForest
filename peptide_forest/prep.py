...
def test_cleavage_aa(
    aa_field: str,
    aa_start: str = None,
    delim_regex: Pattern = None,
    cleavage_sites: Set = None,
) -> bool:
    """
...
def preprocess_df(df: pd.DataFrame) -> pd.DataFrame:
  ...
  """
    ...
    # ^--- maybe a warning ?
    # [TRISTAN] Targets is here for decoy == 1? and vise versa?
...
def get_stats(df: pd.DataFrame) -> Dict[str, Any]:
  ...
  """
    ...
    for engine in engines:
        ...
        # [TRISTAN] 1e-30 willkürlich gewählt?
...
def row_features(
  ...
  """
    ...
    # [TRISTAN] How would that even be possible?
    df[f">Charge{max_charge+1}"] = df["Charge"] > max_charge
    return df


def get_top_targets_decoys(
    df: pd.DataFrame, balance_dataset: bool = False
) -> pd.DataFrame:
    """
...
def col_features(
    df: pd.DataFrame, only_top: bool = False, balance_dataset: bool = False
) -> pd.DataFrame:
    """
