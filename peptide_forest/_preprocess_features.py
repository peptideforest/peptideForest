...
def test_cleavage_aa(aa_field, aa_start=None, delim_regex=AA_DELIM_REGEX, cleavage_sites=CLEAVAGE_SITES):
    """
...
def combine_engine_data(df: pd.DataFrame, feature_cols: List,
  ...
  cols_single = list(['Score_processed', 'delta_score_2', 'delta_score_3',
                      'Mass', 'Delta_m/z', 'absDelta_m/z', 'lnabsDelta_m/z'])
  if keep_ursgal:
  ...
  # columns to group by
  cols_u = ['Spectrum ID', 'Sequence', 'Sequence Start',
            'Sequence Stop', 'Modifications', 'Is decoy', 'Protein ID']
