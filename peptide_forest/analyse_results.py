...
def get_top_targets(df: pd.DataFrame, q_val_cut: float = 0.01) -> pd.DataFrame:
  ...
  for col_target, col_q_val in zip(cols_target,
                                   cols_q_val):
...
def get_ranks(df: pd.DataFrame, method: str = 'first', from_scores: bool = True) -> pd.DataFrame:
  """
...
def get_shifted_psms(df: pd.DataFrame, x_name: str, y_name: str,
                     n_return: int = 10) -> pd.DataFrame:
  """
  ...
  df_new_top_targets = df[~df[tt_x] & df[tt_y]].sort_values(col_x, ascending=False).copy(deep=True)
  df_new_top_targets = df_new_top_targets.reset_index()
  print(f'Number non top targets for {x_name} that are now top targets: {len(df_new_top_targets)}')
  ...
  for i in df_new_top_targets.index:
    ...
    if (df_spectrum[f'Score_processed_{x_name}'] != 0).all():
      df_spectrum = df_spectrum.sort_values(f'Score_processed_{x_name}', ascending=False)
      if any([sequence != df_spectrum['Sequence'].values[0],
  ...
  df_old_top_targets = df[df[tt_x] & ~df[tt_y]].sort_values(col_x, ascending=True).copy(deep=True)
  df_old_top_targets = df_old_top_targets.reset_index()
  print(f'Number top targets for {x_name} that are now not top targets: {len(df_old_top_targets)}')
  if n_return is not None:
  ...
  for i in df_old_top_targets.index:
    ...
    df_spectrum = df[df.loc[:, 'Spectrum ID'] == spectrum_id]
    if (df_spectrum[f'Score_processed_{x_name}'] != 0).all():
      df_spectrum = df_spectrum.sort_values(f'Score_processed_{y_name}', ascending=False)
      if any([sequence != df_spectrum['Sequence'].values[0],
              protein_id != df_spectrum['Protein ID'].astype(str).values[0],
              mods != df_spectrum['Modifications'].values[0]]):
...
def calc_final_q_vals(df: pd.DataFrame, col: str, fac: float = 0.9,
  ...
  df_scores = get_q_vals(df, score_col, fac=fac,
                         top_psm_only=top_psm_only, from_method=from_method)
  df[q_col] = 1.0
...
def calc_all_final_q_vals(df: pd.DataFrame, fac: float = 0.9,
                          top_psm_only: bool = True,
                          from_method: str = None) -> pd.DataFrame:
  """
  ...
  for col in cols:
    df = calc_final_q_vals(df, col, fac=fac, top_psm_only=top_psm_only, from_method=from_method)
  return df
...
def get_protein_maxarea_dataset(df: pd.DataFrame, prot: str) -> pd.DataFrame:
  ...
  df_prot['avg_maxarea'] = df_prot['MAXAREA_msgfplus'].groupby(
    df_prot['Sequence']).transform('mean')
...
def get_num_psms_by_method(df: pd.DataFrame, q_val_cut: float = 0.01,
                           methods: List = None, from_method: str = None) -> pd.DataFrame:
  """
  ...
  df_num_psms = df[[c for c in df.columns if 'top_target_' in c and c in methods]].sum()
  df_num_psms = df_num_psms.to_frame().reset_index()
  df_num_psms.columns = ['method', 'n_psms']
  df_num_psms['method'] = df_num_psms['method'].apply(lambda x: x.split('top_target_')[-1])
  engine_cols = [m for m in df.columns if 'top_target_' in m]
  engine_cols = [m for m in engine_cols if all(ml_method not in m for ml_method in ml_methods)]

  if 'any' not in df_num_psms['method'].values:
    df_num_psms = df_num_psms.append({'method': 'any-engine',
                                      'n_psms': df[engine_cols].any(axis=1).sum()},
                                     ignore_index=True)
  if 'all' not in df_num_psms['method'].values:
    df_num_psms = df_num_psms.append({'method': 'all-engines',
  ...
  if 'majority' not in df_num_psms['method'].values:
    ...
    df_num_psms = df_num_psms.append({'method': 'majority-engines',
                                      'n_psms': n_psms},
                                     ignore_index=True)
...
def get_num_psms_against_q_cut(df: pd.DataFrame, methods: List = None,
                               q_val_cuts: List = None, from_method: str = None) -> pd.DataFrame:
  """
  ...
  df_num_psms_q = pd.DataFrame(index=[str(q) for q in q_val_cuts],
                               columns=methods + ['top_target_any-engine',
                                                  'top_target_all-engines'])
  # get a list of the engine columns
  engine_cols = [f'top_target_{m}' for m in ['msgfplus', 'mascot', 'xtandem', 'omssa']]
...
def get_removed_top_targets(df: pd.DataFrame, method: str,
  ...
  df_removed_top_targets = df[(~df['keep in']) &
                              (~df['Is decoy']) &
                              (df[col_score] >= min_score)]
