...
def read_quant_data(file_name: str) -> pd.DataFrame:
  ...
  df_quant['SPECTRUM_ID'] = df_quant['SPECTRUM_ID'].apply(lambda x: int(str(x).split('F')[-1]))
  cols = ['SPECTRUM_ID', 'MAXAREA', 'ISOTOPELABEL_ID', 'S2I']
  return df_quant[cols]


def get_maxarea_per_spectrum(df: pd.DataFrame, file_name: str,
                             s2i_cut: float = -1.0) -> pd.DataFrame:
  """
  ...
  df_quant = pd.pivot_table(df_quant, values=f'MAXAREA',
                            index=['SPECTRUM_ID'],
                            columns=['ISOTOPELABEL_ID'], aggfunc=np.max)
...
def get_ratios_method(df: pd.DataFrame, i: int, j: int,
                      method: Union[str, Tuple[str, str]] = None,
                      log2: bool = False, drop_no_data: bool = True) -> pd.DataFrame:
  """
  ...
  engines = [e for e in engines if all(ml_method not in e for ml_method in ml_methods)]
  ...
  if isinstance(method, str):
    ...
    df_ratios = df[df[f'top_target_{method[0]}'] & ~df[f'top_target_{method[1]}']].copy(deep=True)
    df_ratios = df_ratios[[f'channel_{i}', f'channel_{j}']]
  else:
    ...
    df_ratios['top_target_majority-engines'] = df_ratios[engines].sum(axis=1) >= n_majority_engines
  ...
  df_ratios['Is human'] = df.loc[df_ratios.index, 'Protein ID'].str.lower().str.contains('human')
  # remove any PSMs where are least 1 channel = 0
  if drop_no_data:
    df_ratios = df_ratios[(df_ratios[[f'channel_{i}', f'channel_{j}']] != 0).all(axis=1)]
  # get the ratios and sums
  df_ratios[f'ratio {i}/{j}'] = (df_ratios[f'channel_{i}']
                                 / df_ratios[f'channel_{j}'])
  df_ratios[f'sum {i}+{j}'] = (df_ratios[f'channel_{i}']
                               + df_ratios[f'channel_{j}'])
...
def get_slice_mean_error(df_input: pd.DataFrame, n_slices: int = 10, is_ecoli: bool = True,
                         quant_cut: bool = True, bins_in: np.array = None,
                         bin_inv_sum: bool = False, bin_log_sum: bool = False,
                         base: int = 2, mean_of_log: bool = False) -> pd.DataFrame:
  """
  ...
  if bins_in is not None:
    ...
    df.loc[:, 'mid'] = pd.cut(df[col_sum], bins=bins_in).apply(lambda x: x.mid).values
    ...
    if quant_cut:
      df.loc[:, 'mid'] = pd.qcut(df[col_sum], n_slices).apply(lambda x: x.mid).values
      df.loc[:, 'bin'], bins_out = pd.qcut(df[col_sum], n_slices, labels=False, retbins=True)
    else:
      df.loc[:, 'mid'] = pd.cut(df[col_sum].dropna(), n_slices).apply(lambda x: x.mid).values
      df.loc[:, 'bin'], bins_out = pd.cut(df[col_sum], n_slices, labels=False, retbins=True)
  df_bins = (df[['bin', 'mid']].drop_duplicates()
             .dropna().sort_values('mid').reset_index(drop=True))
  if mean_of_log:
    df.loc[:, f'log{base}({col_ratio})'] = df[col_ratio].apply(lambda x: np.log(x) / np.log(base))
    col_ratio = f'log{base}({col_ratio})'
  df_means = (df.groupby('bin')[col_ratio].agg(['mean', 'std', 'count'])
  ...
  df_means = df_means.rename({'mid': col_sum,
                              'mean': f'<{col_ratio}>',
                              'std': 'error'}, axis=1)
  return df_means, bins_out


def get_logn_mean_error(df_input: pd.DataFrame, n_slices: int = 10, is_ecoli: bool = True,
                        quant_cut: bool = True, bins_in: np.array = None,
                        base: int = 2,
                        bin_inv_sum: bool = False, bin_log_sum: bool = False,
                        mean_of_log: bool = False) -> pd.DataFrame:
  """
  ...
  Returns:
    ...
    - bins_out: bins used for the calculations
  """
  df_means, bins_out = get_slice_mean_error(df_input, n_slices=n_slices,
                                            is_ecoli=is_ecoli,
                                            quant_cut=quant_cut, bins_in=bins_in,
  ...
  if mean_of_log:
    df_means = df_means.rename({'error': f'error [{col_ratio}]'},
                               axis=1)
  else:
    df_means.loc[:, 'error'] = df_means['error'] / (df_means[col_ratio] * np.log(base))
    df_means[col_ratio] = np.log(df_means[col_ratio]) / np.log(base)
    df_means = df_means.rename({col_ratio: f'log{base}({col_ratio})',
...
def fit_function(df_means: pd.DataFrame, consts_guess: List = None,
                 func_to_fit: str = 'arctan') -> Tuple[float, float, List]:
  """
...
def get_maxarea_per_protein(df: pd.DataFrame, target_col: str,
                            s2i_cut: float = None) -> pd.DataFrame:
  """
  Get the maxarea values for each peptide for each protein
  Arguments:
...
def merge_results_quant(file_name: str, df: pd.DataFrame) -> pd.DataFrame:
  ...
  df_quant_training = df_quant.merge(df,
                                     left_on=['SPECTRUM_ID'],
                                     right_on=['Spectrum ID'])
  ...
  df_quant_training = df_quant_training['Protein ID'].apply(pd.Series).merge(df_quant_training,
                                                                             right_index=True,
                                                                             left_index=True)

  # melt the dataframe, to get data for each protein
  df_quant_training = df_quant_training.melt(id_vars=cols, value_name="protein")
  df_quant_training = df_quant_training.drop("variable", axis=1).dropna(subset=['protein'])
  # columns to keep
  cols = ['ISOTOPELABEL_ID', 'protein', 'Spectrum ID', 'Sequence', 'MAXAREA', 'S2I'] + methods
  df_quant_training = df_quant_training[cols]

  df_quant_training['sum_top_targets'] = df_quant_training[methods].sum(axis=1)
  df_quant_training = df_quant_training.sort_values(['protein', 'Sequence',
                                                     'ISOTOPELABEL_ID', 'sum_top_targets'],
                                                    ascending=False)
  df_quant_training = df_quant_training.drop_duplicates(['protein', 'Sequence', 'ISOTOPELABEL_ID'])
...
def get_results_protein(file_name: str, df: pd.DataFrame) -> pd.DataFrame:
  ...
  for method in methods:
    if df_quant_results is not None:
      df_quant_results_sub = get_maxarea_per_protein(df_quant_training, f'top_target_{method}')
      df_quant_results = df_quant_results.merge(df_quant_results_sub, on=group_cols, how='outer')
      df_quant_results = df_quant_results.rename({'MAXAREA': f'MAXAREA_{method}'}, axis=1)
    else:
      # get all top targets from msgfplus
      df_quant_results = get_maxarea_per_protein(df_quant_training, f'top_target_{method}')
      df_quant_results = df_quant_results.rename({'MAXAREA': f'MAXAREA_{method}'}, axis=1)
...
def get_unique_sequence_counts_all(df: pd.DataFrame) -> pd.DataFrame:
  ...
  for col in cols:
    ...
    if df_seq_counts is not None:
      df_seq_counts = df_seq_counts.merge(get_unique_sequence_counts(df, col),
                                          on='protein', how='outer')
      df_seq_counts = df_seq_counts.rename({'Sequence': f'n_sequences_{method}'}, axis=1)
    else:
...
def calc_channel_ratios(df: pd.DataFrame, prot: str, method: str = 'msgfplus') -> pd.DataFrame:
  """
  ...
  df_prot = pd.pivot_table(df_prot, values=f'MAXAREA_{method}', index=['Sequence'],
                           columns=['ISOTOPELABEL_ID'], aggfunc=np.sum)
...
def calc_mean_channel_ratio_diff(df: pd.DataFrame, prot_list: List) -> pd.DataFrame:
  ...
  for prot in prot_list:
    ...
    test1 = test1.rename({'mean_x': 'mean_msgfplus',
                          'var_x': 'var_msgfplus',
                          'mean_y': 'mean_SVM',
                          'var_y': 'var_SVM'}, axis=1)
    ...
    test2 = df_prot[cols].agg(['mean', 'var']).T
    test1 = test1.merge(test2, left_index=True, right_index=True, how='outer')
    test1 = test1.rename({'mean': 'mean_RF',
                          'var': 'var_RF'}, axis=1)
    ...
    df_output.loc[prot,
                  [f'diff_var_SVM_{c}'
                   for c in test1.index]] = test1['diff_var_SVM'].values

    df_output.loc[prot,
                  [f'diff_var_RF_{c}'
                   for c in test1.index]] = test1['diff_var_RF'].values

  return df_output


def calc_all_ratios(df: pd.DataFrame, i: int, j: int, prots: List, methods: List) -> Dict:
  """
  ...
  cols = [f'ratio_{i}_{j}_{m}' for m in methods] + [f'sum_{i}_{j}_{m}' for m in methods]

  # set a dictionary to store the data
  data_ratios = {'Human': {}, 'E-Coli': {}}  # type: Dict[str, Dict[str, pd.DataFrame]]
  ...
  for prot in prots:
    ...
    for method in methods:
      ...
      df_prot = calc_channel_ratios(df, prot,
                                    method=method).sort_values('Sequence')
      ...
      if vals is not None:
        vals = vals.merge(df_prot[['Sequence', i, j, ratio]],
                          on='Sequence', how='outer')
      else:
        vals = df_prot[['Sequence', i, j, ratio]]
      # change the column names
      vals = vals.rename({i: f'{i}_{method}',
                          j: f'{j}_{method}',
                          f'ratio_{i}_{j}': f'ratio_{i}_{j}_{method}'}, axis=1)
      # calculate the sum
      vals[f'sum_{i}_{j}_{method}'] = vals[f'{i}_{method}'] + vals[f'{j}_{method}']
    if not no_data_flag:
      data_ratios[prot_type][prot] = vals[cols]
      data_ratios[prot_type]['all_data'] = pd.concat([data_ratios[prot_type]['all_data'],
                                                      data_ratios[prot_type][prot]],
                                                     ignore_index=True)
  return data_ratios


# pylint: disable=too-many-locals
def get_ratio_against_sum(df: pd.DataFrame, sum_cut: float = 0.0,
                          ...
                          icol: int = 63, jcol: int = 69) -> pd.DataFrame:
  """
  ...
  if q_val_cuts is None:
    q_val_cuts = sorted([float(f'{i}e-{j}') for i in np.arange(1, 10)
                         for j in np.arange(3, 1, -1)]) + [1e-1]

  # get methods
  if methods is None:
    methods = [c.split('top_target_')[-1] for c in df.columns if 'top_target' in c]

  columns = sorted([f'log2(<ratio {icol}/{jcol}>) [sum->infty]_{m}'
                    for m in methods + [method_0]])
  ...
  for q_val_cut in q_val_cuts:
    ...
    # get values for method_0
    mean_ratio_plateau, error, bins = get_mean_error(df,
                                                     sum_cut=sum_cut,
                                                     method=method_0,
                                                     icol=icol, jcol=jcol,
                                                     quant_cut=quant_cut,
    ...
    df = get_top_targets(df, q_val_cut=q_val_cut)
    for method in methods:
      mean_ratio_plateau, error = get_mean_error(df,
                                                 sum_cut=sum_cut,
                                                 method=method,
                                                 icol=icol, jcol=jcol,
                                                 quant_cut=quant_cut,
...
# pylint: disable=too-many-locals
def get_mean_error(df: pd.DataFrame, method: str, icol: int, jcol: int,
                   bin_log_sum: bool, sum_cut: float,
                   mean_of_log: bool, quant_cut: bool,
                   n_slices: int, bins: List = None,
                   method_0: str = None) -> pd.DataFrame:
  """
