...
def get_q_vals(df: pd.DataFrame, score_col: str, fac: float = 0.9,
               top_psm_only: bool = True, get_fdr: bool = True,
               from_method: str = None) -> pd.DataFrame:
  """
  ...
  engines = [e for e in engines if e not in [f'Score_processed_{m}' for m in ml_methods]]
  ...
  if from_method and not not_in_engines:
    df_scores = df.sort_values([score_col, f'Score_processed_{from_method}'],
                               ascending=[False, False]).copy(deep=True)
  else:
    df_scores = df.sort_values(score_col,
  ...
  df_scores.loc[:, str(target_decoy_dummies.columns[0])] = target_decoy_dummies.values[:, 0]
  df_scores.loc[:, str(target_decoy_dummies.columns[1])] = target_decoy_dummies.values[:, 1]
  df_scores = df_scores.rename({'False': 'Target',
                                'True': 'Decoy'}, axis=1)
...
def find_psms_to_keep(df: pd.DataFrame, score_col: str) -> pd.DataFrame:
  ...
  df_with_top_val = df[df['Spectrum ID'].isin(inds) &
                       df[eq_col]].copy(deep=True)
  # check how many decoys there are for each spectra
  df_num_decoys = df_with_top_val.groupby('Spectrum ID')['Is decoy'].agg(['count',
                                                                          'sum',
                                                                          'nunique'])
  ...
  psms_keep = list(df[df['Spectrum ID'].isin(inds_keep) & 
                      df[eq_col]].sample(frac=1.0)
                   .drop_duplicates('Spectrum ID').index)

  # and drop the other one
  psms_drop = list(df[df['Spectrum ID'].isin(inds_keep) & 
                      df[eq_col] & 
                      (~df.index.isin(psms_keep))].index)
...
def get_top_targets(df: pd.DataFrame, score_col: str, q_cut: float = 0.01,
                    fac: float = 0.9) -> pd.DataFrame:
  """
  ...
  """
  df_scores = get_q_vals(df, score_col, fac=fac, top_psm_only=False,
                         get_fdr=False)
  inds = df_scores[(df_scores["q-value"] <= q_cut) & (~df_scores["Is decoy"])].index
  return df.loc[inds, :]


def get_train_set(df: pd.DataFrame, score_col: str, q_cut: float = 0.01,
                  fac: float = 0.9, train_top_data: bool = False,
  ...
  if n_sample > len(df_to_train[df_to_train["Is decoy"]]):
    train_decoys = df_to_train[df_to_train["Is decoy"]].sample(n=n_sample, replace=True)
  else:
...
def get_train_test_sets(df: pd.DataFrame, use_cv: bool = True) -> List:
  ...
  if use_cv:
    ...
    train_decoys, test_decoys = model_selection.train_test_split(df[df["Is decoy"]],
...
def get_scores(scores_in: Dict, clf: Any, norm_data: pd.DataFrame,
               data: pd.DataFrame, split_name: str) -> Dict:
  """
...
def calc_num_psms(df: pd.DataFrame, score_col: str, q_cut: float = 0.01,
                  fac: float = 0.9) -> int:
  """
  ...
  n_psms = len(df_scores_sub.loc[
    (df_scores_sub["q-value"] <= q_cut) & (~df_scores_sub["Is decoy"]), :])
  return n_psms


# pylint: disable=too-many-locals,too-many-branches,too-many-statements
def fit_model_cv(model_type: str,
                 features: List,
                 df: pd.DataFrame,
                 n_train: int = 10, n_eval: int = 15,
                 ...
                 fac: float = 0.9) -> Tuple[Any, Dict, Dict, Dict,
  ...
  for i_it in range(n_iter):
    ...
    for it in np.arange(len(training_data)):
      ...
      train_j = training_data[j][list(org_cols.union([score_col]))].copy(deep=True)
      train_k = training_data[k][list(org_cols.union([score_col]))].copy(deep=True)

      # fill in missing values
      train_j, train_k, test = setup_dataset.replace_missing_data_cv(train_j, train_k, test)
      
      # train on 2/3 data, with top targets and 50% decoys
      train = pd.concat([train_j, train_k])
      train = get_train_set(train, score_col, fac=0.9, train_top_data=train_top_data,
                            q_cut=q_cut_train)
      ...
      training_data[j].loc[:, 'model_score_train'] = clf.score_psm(train_j[features])
      training_data[k].loc[:, 'model_score_train'] = clf.score_psm(train_k[features])
      df.loc[training_data[it].index, 'model_score'] = training_data[it]['model_score']
      # get the training score, which we take as the average score over each time the
      # psm is trained on
      df.loc[train_j.index, 'model_score_train'] += training_data[j]['model_score_train'].values
      df.loc[train_k.index, 'model_score_train'] += training_data[k]['model_score_train'].values
    df.loc[:, 'model_score_train'] = df['model_score_train'] / (len(training_data) - 1)
    ...
    if i_it >= n_train:
      ...
      df.loc[:, 'model_score_train_all'] += df['model_score_train'].apply(lambda x: [x])
      df.loc[:, 'model_score_train_avg'] = df['model_score_train_all'].apply(np.mean)

    # see how many PSMs are below 1% in the test and train set
    psms['test'].append(calc_num_psms(df, 'model_score', q_cut=q_cut, fac=fac))
    psms['train'].append(calc_num_psms(df, 'model_score_train', q_cut=q_cut, fac=fac))

    if i_it >= n_train:
      psms_avg['test'].append(calc_num_psms(df, 'model_score_avg', q_cut=q_cut, fac=fac))
      psms_avg['train'].append(calc_num_psms(df, 'model_score_train_avg', q_cut=q_cut, fac=fac))

    if verbose == 2:
      print(f'Iteration {i_it + 1: >3}', i_it + 1, psms['train'][i_it], psms['test'][i_it])
  if verbose in [1, 2]:
  ...
  df_feature_importance = pd.DataFrame({'feature_importance': feature_importance,
...
# pylint: disable=too-many-locals,too-many-branches,too-many-statements
def fit_model_at(model_type: str,
                 features: List,
                 df: pd.DataFrame,
                 n_train: int = 10, n_eval: int = 15,
                 hp_dict_in: Dict = None,
  ...
  # get number of PSMs with q-value<1% using search engine score
  psms_engine['train'] = calc_num_psms(training_data[0], initial_score_col, q_cut=q_cut, fac=fac)
  psms_engine['test'] = calc_num_psms(training_data[1], initial_score_col, q_cut=q_cut, fac=fac)
  ...
  for i_it in range(n_iter):
    ...
    train = get_train_set(training_data[0].copy(deep=True),
                          score_col,
                          fac=0.9,
    ...
    # pylint: disable=not-callable
    training_data[0].loc[:, 'model_score_train'] = clfs_sub.score_psm(train_all[features])
    df.loc[test.index, 'model_score'] = training_data[1]['model_score']
    # pylint: disable=not-callable
    df.loc[train_all.index, 'model_score_train'] = clfs_sub.score_psm(train_all[features])
...
def score_from_model(df: pd.DataFrame, features: List, classifiers: List) -> pd.DataFrame:
  """
...
def fit_model(model_type: str,
  ...
  if initial_score_col is None:
    initial_score_cols = [score_col for score_col in features if 'Score_processed_' in score_col]
    initial_score_cols = [score_col for score_col in initial_score_cols
  ...
  for initial_score_col in initial_score_cols:
    ...
    if use_cv:
      (clfs[initial_score_col],
       ...
       df,
       feature_importances[initial_score_col]) = fit_model_cv(model_type,
                                                              features,
                                                              df,
                                                              n_train=n_train,
                                                              n_eval=n_eval,
      ...
      (clfs[initial_score_col],
       ...
       psms_engine[initial_score_col],
       df,
       feature_importances[initial_score_col]) = fit_model_at(model_type,
                                                              features,
                                                              df,
                                                              n_train=n_train,
                                                              n_eval=n_eval,
                                                              ...
                                                              q_cut_train=q_cut_train,
                                                              fac=fac)
    df[f'score_{model_type}_from_{initial_score_col}'.lower()] = df[f'Score_processed_{model_type}']

  df = df.drop(f'Score_processed_{model_type}', axis=1)
  cols = [f'score_{model_type}_from_{initial_score_col}'.lower() for initial_score_col
          in initial_score_cols]
